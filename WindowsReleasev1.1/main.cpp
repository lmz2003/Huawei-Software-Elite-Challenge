...
void Update()
{
    ...
    for(int i = 0; i < robot_num; i ++)
    {
        ...
        if(robot[i].goods)
        {
            ...
            if(next_point.second.first-robot[i].x == 1) printf("move %d 3\n",i);			//更新机器人的坐标  输出指令
            else if(next_point.second.first-robot[i].x == -1) printf("move %d 2\n",i);
            else if(next_point.second.second-robot[i].y == -1) printf("move %d 1\n",i);
            else if(next_point.second.second-robot[i].y == 1) printf("move %d 0\n",i);
