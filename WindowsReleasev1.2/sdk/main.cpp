    ...
    for(int i=0;i<5;i++){
        ...
        if(boat[i].status == 0) continue;		//船处于运输状态
            ...
            if(goods_get >= berth[boat[i].pos].hasgoods ) {		//如果船上货物数量等于船的容量，则船运往虚拟点
				...
				if(berth[boat[i].pos].hasgoods + boat[i].boatgoods< boat_capacity-10 ){
				...
				berth[boat[i].pos].hasgoods = berth[boat[i].pos].hasgoods-(boat_capacity-boat[i].boatgoods);
	            boat[i].arrival_time = 0;
	            berth[boat[i].pos].isboatcoming = 0;
	            boat[i].boatgoods = 0;
	            printf("go %d\n",i);
			...
			else if(id + berth[boat[i].pos].transport_time > 14998){
		        berth[boat[i].pos].hasgoods = 0;
		        boat[i].arrival_time = 0;
		        berth[boat[i].pos].isboatcoming = 0;
