    ...
    for(int i=0;i<5;i++){
        ...
        if(boat[i].status == 1 && boat[i].pos == -1){    //船位于虚拟点，准备前往泊位   或船在泊位外等待
            ...
            if(goods_get >= berth[boat[i].pos].hasgoods ) {		//如果船上货物数量等于船的容量，则船运往虚拟点
				...
				if(berth[boat[i].pos].hasgoods + boat[i].boatgoods< boat_capacity-10 && id < 13598){ 
				...
				else if(id >= 13598 && id + berth[boat[i].pos].transport_time <= 14996)
				{
					continue;
				}
				else {
			...
			else if(id + berth[boat[i].pos].transport_time > 14996){
		        berth[boat[i].pos].hasgoods = 0;
		        boat[i].arrival_time = 0;
		        berth[boat[i].pos].isboatcoming = 0;
...
{
    for(int i = 0;i < robot_num;i++)
        ch[next_position[i].first][next_position[i].second] = '.';
    //更新机器人状态
...
//    	ch[now_position[i].first][now_position[i].second] = '#';  //将当前机器人位置设为障碍
		...
		if(wait[i]>9){             //如果机器人卡住不动，过5帧就让他随机移动
			while(1){
//				int dir = (to_dir+3)%4;
//				to_dir = (to_dir+3)%4;
				int dir = rand()%4;
				...
				ch[robot[i].x + dx[dir]][robot[i].y + dy[dir]] = '#';
				next_position[i] = {robot[i].x + dx[dir],robot[i].y + dy[dir]};
        ...
        if(robot[i].goods)
    	{
            ...
            {
                if(id > (15000- 2*berth[k].transport_time) && berth[k].isboatcoming == 0) continue;
            ...
            {
                if(j == i) continue;
                if(next_position[j] == next_point){
                    next_point = {robot[i].x, robot[i].y};
			...
			if(flag == 0){
				next_position[i] = next_point;
            	ch[next_point.first][next_point.second] = '#'; //将机器人即将要走的位置也设为障	
			}
            ...
            for(auto good:goods){
                ...
                if(id < 9900) dcost = 2*dist[good.x][good.y]- good.value - 0.105*(id - good.startid); //10000-13000
